...
EXPERIMENT_DOMAINS = ["숙소", "지하철", "식당", "관광", "택시"]
...
def load_dataset(dataset_path, dev_split=0.1):
    ...
    num_per_domain_trainsition = [int(num_dev / k) for k in dom_mapper.keys()]
    dev_idx = []
    for i, v in enumerate(dom_mapper.values()):
        idx = random.sample(v, num_per_domain_trainsition[i])
        dev_idx.extend(idx)
...
def prepare_dataset(origin_dials, tokenizer, slot_meta,
