...
def load_dataset(dataset_path, dev_split=0.1):
    ...
    for d in data:
        domain = "_".join(sorted(d["domains"]))
        dom_mapper[domain].append(d["dialogue_idx"])

    dev_idx = []
    for v in dom_mapper.values():
        num_to_pick = int(len(v) * dev_split)
        idx = random.sample(v, num_to_pick)
        dev_idx.extend(idx)
...
def load_dataset(dataset_path, dev_split=0.1):
    ...
    # domain 개수별로 stratified 추출
    # total = len(dom_mapper[1]) + len(dom_mapper[2]) + len(dom_mapper[3])
    dev_idx = []
    # domain 종류 자체도 고려
    for k, v in dom_mapper.items():
        num_per_domain_trainsition = int(num_dev * (len(dom_mapper[k])/total))
...
def count_slot_meta(data):
    slot_meta = {slot : 0 for slot in build_slot_meta(data)}
    for dialog in data:
        for turn in dialog["dialogue"]:
            if not turn.get("state"):
                continue

            dom_slot_value = turn["state"][-1]
            domain_slot, _ = split_slot(dom_slot_value, get_domain_slot=True)
            if domain_slot not in slot_meta:
                slot_meta[domain_slot] = 1
            else :
                slot_meta[domain_slot] += 1
    return slot_meta
