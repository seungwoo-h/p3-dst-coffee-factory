...
class Processor(object):
    def __init__(self, config):
        with open('/opt/ml/input/data/train/ontology.json', 'r', encoding='utf-8') as f:
    ...
    def get_train_instances(self, data_dir, tokenizer):
        return self._create_instances(json.load(open('/opt/ml/input/data/train/train_splited.json')), tokenizer)

    def get_dev_instances(self, data_dir, tokenizer):
        return self._create_instances(json.load(open('/opt/ml/input/data/train/dev_splited.json')), tokenizer)

    def get_test_instances(self, data_dir, tokenizer):
        return self._create_instances(json.load(open('/opt/ml/input/data/eval/eval_dials.json')), tokenizer)
    ...
    def _create_instances(self, lines, tokenizer):
        ...
        for line in lines:
            dialogue_idx = line['dialogue_idx']
            system_response = ''
            last_dialogue_state = {slot: 'none' for slot in self.slot_meta}
            history_uttr = []
            last_uttr = ""
            is_last_turn = False

            for ti, turn in enumerate(line['dialogue']):
                if turn['role'] == 'sys':
                    system_response = turn['text']
                    continue
                turn_idx = ti // 2
                is_last_turn = (ti == len(line) - 1)
                user_utterance = turn['text']
                turn_dialogue_state = {}
                turn_dialogue_state_ids = []

                if turn.get('state') is not None:
                    state = {slot: val for slot, val in map(lambda x: x.rsplit('-', 1), turn['state'])}
                else:
                    state = {}

                for slot_idx, slot in enumerate(self.slot_meta):
                    value = state[slot] if state.get(slot) is not None else 'none'
                    turn_dialogue_state[slot] = value
                    turn_dialogue_state_ids.append(self.label_map[slot_idx][value])
